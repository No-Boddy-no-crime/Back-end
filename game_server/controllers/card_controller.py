...
def pick_casefile(game_id):
    ...
    casefile = [guilty_room, guilt_weapon, guilty_person]
    db.update_game(game_board_id=game_id, new_game_state={"casefile": casefile})
    return casefile

def deal_remaining_cards(game_board_id, casefile):
    remaining_cards = list(CARDS.difference(set(casefile)))
    print("cards", len(remaining_cards))
    ...
    print("remain:" , remaining_cards, visible_cards)
    assert len(remaining_cards) == visible_cards
    db.update_game(game_board_id=game_board_id, new_game_state={"players": players, "visible_cards": remaining_cards})
